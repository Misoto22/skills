...
Make every substantive paragraph conditional and cite one or more ledger evidence IDs inline. A citation is the bracketed token that opens the ledger entry — `[E-ASPECT-9DAF]`, `[E-OVERLAY-3096]` — copied character for character into the sentence it supports; the display string beside it is not a substitute for the token. Conditional means the paragraph carries one of `can`, `could`, `may`, `might`, `tends`, `often`, `suggests`, `appears`, `possibly`, `perhaps`, `potentially`, or in Chinese 可能, 也许, 或许, 倾向, 往往, 通常, 有时, 似乎. `will`, `shall`, `guarantees`, `destined`, `definitely`, `inevitably`, `certain to`, `must happen`, `going to`, 必然, 注定, 保证, 一定会, 肯定会 and 必定 are refused wherever they appear, including in prose that was not meant as a forecast. Copy evidence IDs, body ownership, directions, aspect kinds, exact orbs, orb ranges, houses, and certainty labels exactly. Describe uncertain aspects only as `confirmed` or `possible` with their recorded range. Do not fabricate overlays for `window` or `date-only` charts.
