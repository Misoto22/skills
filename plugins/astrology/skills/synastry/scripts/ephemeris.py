...
def resolve_subject(
    ...
    julian_days = tuple(_julian_day(moment, swe) for moment in moments)
    interval = replace(interval, julian_start=julian_days[0], julian_end=julian_days[-1])
    ...
    for julian_day in julian_days:
        for name, code in bodies.items():
            if name in unavailable:
                continue
            try:
                position, flags = swe.calc_ut(julian_day, code, requested_flags)
            except swe.Error as error:
            ...
            returned_flags.add(int(flags))
            actual_backends.add(actual)
            samples = collected[name]
            samples.longitudes.append(float(position[0]) % 360.0)
            samples.latitudes.append(float(position[1]))
            samples.distances.append(float(position[2]))
            samples.speeds.append(float(position[3]))
    ...
    if isinstance(subject.birth, ExactBirth):
        houses, angles = _resolve_houses(julian_days[0], subject.birth, options, swe)
...
def _resolve_houses(
    ...
    try:
        raw_cusps, raw_angles = swe.houses(
            julian_day,
            birth.latitude,
            birth.longitude,
    ...
    if len(cusps) != 12:
        raise EphemerisError(f"house calculation returned {len(cusps)} cusps instead of twelve")
    ascendant = float(raw_angles[0]) % 360.0
    medium_coeli = float(raw_angles[1]) % 360.0
    angles = {
        "ascendant": ascendant,
        "medium_coeli": medium_coeli,
        "descendant": (ascendant + 180.0) % 360.0,
        "imum_coeli": (medium_coeli + 180.0) % 360.0,
        "vertex": float(raw_angles[3]) % 360.0,
        "east_point": float(raw_angles[4]) % 360.0,
    }
    return cusps, angles
