# Sync — iteration 2 gate

## The edit

One replace, of one contiguous block: the `description` line in
`plugins/dev/skills/sync/SKILL.md`.

| | |
| --- | --- |
| before | 435 characters |
| after | 321 characters |
| removed | 114 |

Three things went, and the body owns all three:

- **The triple negation.** "It never rebases a feature branch, never resolves a
  conflict, and never discards a local commit." It reads "It only fast-forwards;
  anything diverged is reported for the user to decide" now — the same contract,
  stated as what the skill does rather than as three things it must not.
- **The boundary written twice.** "never resolves a conflict" and "Not for …
  resolving a merge conflict" were forty characters apart. The `Not for` clause
  keeps it.
- **Two of the three ways of saying pull-latest.** `更新到最新` and `更新一下代码`
  went; `拉一下最新的` stayed, because that is the one a held-out case lands on.
  "update from main" and "catch up with remote" collapsed into "get up to date
  with the remote".
