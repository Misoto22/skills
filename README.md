...
Twenty-four skills in seven plugins. The plugin name is the command prefix, and each plugin installs on its own — a plugin is a subject, not a bucket.
...
- **[reunite](plugins/dev/skills/reunite/SKILL.md)** (`/dev:reunite`) — makes every signed-in account see every conversation. The desktop app keeps one sidebar index per account, so signing in as a second account hides history it never deleted — the transcripts carry no account field at all. This unions the indexes and pulls a shared conversation's diverged titles back onto one name, only ever adding entries, records what it wrote and what it renamed, and `--undo` takes exactly those back.
...
Claude Code and Codex both install these. `npx skills add` and the skills.sh listing do not show them at all: they clone this repository and read the skills on disk, so a plugin that lives in someone else's repository is not theirs to offer. Those two routes stay at the twenty skills above.
